...
마크다운 파일을 로컬이 아닌 서버에서 fetch하여 사용한다. 물론 Node의 fs 모듈을 이용해 로컬 파일을 읽어온 후 next-mdx-remote의 serialize 함수를 써서 파싱하는 방법도 있지만, 이 경우 자동 파일 기반 라우팅이 지원되지 않아 MDX 콘텐츠를 따로 props로 전달하는 추가 작업이 필요하다. 그래서 설정 복잡성이나 유지보수 측면에서는 @next/mdx보다 불편하게 느껴질 수밖에 없다.

따라서 @next/mdx 라이브러리를 사용하기로 결정했다!

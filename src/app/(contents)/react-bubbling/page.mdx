export const meta = {
  emoji: "🫡",
  title: "키오스쿨 실시간 주문 상세 모달 버그 해결기",
  date: "2025-04-17",
...
```tsx,title=OrderDetailModalButton
...
`useModal`훅에서 모달 관련 함수 및 변수들을 받아와서 사용하고 있다. 이제 `useModal` 훅을 살펴보자.

```tsx,title=useModalHook
function useModal() {
...
`openModal`에서 `overflow: hidden`을 주고, `closeModal`에서 `overflow: auto`를 주고 있다. 스크롤이 잠긴다는 건, `closeModal`이 호출되지 않았다는 의미다. 확인해보니, 주문 상태 변경 버튼에 `closeModal` 호출이 빠져 있었다. 이를 추가하니 첫번째 버그는 해결이 됐다!

## 모달을 트리거하는 범위를 확장해 달라는 요청

추가적으로 ‘>’ 아이콘에서 `OrderCard` 컴포넌트 주요 영역 전체로 클릭 범위를 넓여달라는 요구가 있었다.
